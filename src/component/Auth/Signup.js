...
function Signup() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (isChecked) {
      try {
        ...
        setIsLoading(false);
        setMessage("Sorry for the inconvenience, please try again.");
      }
  ...
  const handleVerify =  async (event) => {
    ...
    try {
      const response = await axios.post(`${url}/auth/verifyCode`, user);
      console.log(response.data);
      ...
      setIsLoading(false);
      setMessage("Your profile Couldn't be created now. Please check your internet and try agin later");
    }
  ...
  return (
    ...
    {!showVerify &&(  <section>
      ...
      <p className='font-bold text-lg'>Or click here to   <Link to='/signIn'><a className='text-[#4747ff]' href='/signIn' >sign in</a></Link>  if you already have an account</p>
      {message}
      </div>
    ...
    >
      ...
      {showVerify && (
        ...
        <form className='mt-10' onClick={handleVerify}>
        <div>
